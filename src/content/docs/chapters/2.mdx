...
1. Create a `.gitignore` file to prevent unwanted files from being checked-in to VCS.
    ...
    I'd recommend this as a `.gitignore`.
    ```txt title=".gitignore"
...
Now we need to split up our NixOS config from one file, this will make it easier to find where we set options as they'll be categorized.
...
You'll also need to edit the path specified in `flake.nix` within the `modules` array to point to `./modules/config.nix` instead.
...
Since NixOS uses nixpkgs already, it's already passed to us as an argument in modules. However, there may come a time that we need to access other inputs within our configuration. 

In order to do this we have to tell NixOS additional arguments we want passed to modules.
