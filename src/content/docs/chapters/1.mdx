...
After installing check that you can run Nix by running `nix --version`.
...
To start out make a new folder to contain this project. I'd recommend somewhere easy to access via terminal. In this new folder initialize a new Nix flake.
...
2. We have another attribute called `inputs` that's another attr set. 
   This attr set contains an attribute named `nixpkgs` which is another attr set.
   Within this `nixpkgs` attr set we set `url` to `github:nixos/nixpkgs?ref=nixos-unstable`
3. We have a final attribute named `outputs` that's a *function*.
...
This is the basic structure of a flake. We have `inputs`, which define dependencies for or flake. And we have `outputs`, which is a function that can be passed the inputs we defined to output *something*. 
...
<Steps>
1. Remove the current template lines that output `hello`
...
We want our flake to export this expression so we can use it in the Nix CLI.
...
1. First we'll add the output. Note that the start of the flake file isn't included here for brevity
    ```nix title="flake.nix" "YOURNAME" ins={10-12}
    { 
      outputs = { 
        self, 
        nixpkgs 
...
> TODO: Gif(?) of VM launching and logging in with test user
...
> TODO: Image - XFCE greeter(?) showing test user
