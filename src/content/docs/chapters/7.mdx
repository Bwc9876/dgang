...
> TODO: IMAGE NIXOS INSTALLER BOOT
...
First things first we need to get the main disk ready for NixOS. To do this we *will have to wipe it*.
...
In this TUI you'll need to do a few things:
...
3. Next you can optionally set swap memory, if you do set this I'd recommend making it the size of physical memory you have
   ...
   1. New partition
   2. Press Enter
   3. Enter the size you desire for the swap partition.
...
4. Finally allocate the rest of the disk space to your actual system
   ...
   4. For hex code press enter (`8300`), for a Linux filesystem
   5. You can name this `NIXOS` or whatever you want
   </Steps>
5. Finally select `[Write]` at the bottom, this will commit all changes to the disk
6. Quit
</Steps>
...
- For drive it should be at `/dev/sdX#` where `#` is replaced with the partition number
...
3. **If you did swap**, Enable swapping
   ...
   ```
3. Setup your *main partition* (`3` **if you did swap**, `2` otherwise)
...
Next we need to mount the disks we just created for install.
...
Almost ready now. But first, we'll copy our system flake to this directory.
...
1. Make the folder to store the flake in
   ...
   ```
2. Copy the flake to our newly mounted drive
   ```sh
...
The layout of `/mnt` should now look like
...
Now we need to add hardware-specific modules to our NixOS config for this computer.
...
After rebooting the system should go into a menu with `NixOS` and `Reboot into firmware interface`. We want `NixOS`.

The system should initialize and reach some sort of greeter (`tuigreet`, `SDDM`, `GDM`, etc depending on what you set up). 

Before attempting to login (you can't, you don't have a password). We need to set your user password

<Steps>
1. Switch TTYs with `Ctrl+Alt+F2`. (You may need to hold `Fn` as well on a laptop)
2. Login as `root` with the password you setup in the installer
3. Run `passwd YOURNAME`, replacing `YOURNAME` with your normal user's name
4. Enter the password you wish to use 
5. Logout with `exit` or press `Ctrl+D`
6. Switch back to TTY 1 with `Ctrl+Alt+F1`
</Steps>

Now log in to your new system!

After logging in I'd recommend grabbing your flake from `/flake` and copying it to somewhere in your home folder for editing. The flake will no longer have a git repo associated with it so you may need to re-run `git init`. Either way make sure to commit your new `hardware-configuration.nix` file and push if applicable.

You can remove `/flake` once you're done this.

## Post-Install Config Housekeeping

First thing, we don't need QEMU guest agent anymore as we're on the read deal now
...
```nix title=flake.nix ins={5} del={4}
modules = [
   ./config.nix
   # ...
   ./hardware-configuration.nix
...
Switching is the process of pointing your running system to another config
...
Nix can automatically update all inputs to their latest commits with
...
When booting, `systemd-boot` will display all previous NixOS generations, you can select one to boot from. This is very handy if you mess up your most recent config to be unbootable. With NixOS as long as you don't mess up your bootloader you can usually recover easily.

> TODO: IMAGE OF systemd-boot with previous generations
