...
<Aside>
Like a NixOS module, we can define packages inline or in a separate file as a function. Most people prefer to do it as a separate file.
</Aside>
...
This derivation contains everything the Nix build system needs to build the package: which other packages are needed, what commands to run, environment variables to set, etc.
...
"/nix/store/": {
  ...
  "Package Output" -> "Dependencies": Symlinks to if needed
}
...
You can check on both your host system and VM for the existence of this directory and see it has *many* items. You'll also notice the directories all have a hash after the name, this is so no conflicts exist when building two version of the same package.
...
"/": {
  ...
  "/run/current-system/sw" {shape: package}

  "/etc/[some stuff]" -> "/run/current-system/sw": "+/etc"
}
...
"/"."/run/current-system/sw" -> "/nix/store"."/[hash]-system-path": Symlink
...
"Environment Variables"."..." -> "/"."/run/current-system/sw"
```
...
```d2
"/run/current-system/sw" -> "[hash]-system-path": Old link severed {
  target-arrowhead: {shape: diamond}
  style.stroke-dash: 5
}
"/run/current-system/sw" -> "[different-hash]-system-path": New link created! {
  target-arrowhead: {
    shape: diamond 
    style.filled: true
  }
...
<Aside>
This diagram is a simplification, switching `/run/current-system/sw` is only one part of switching to a new config.
...
`cache.nixos.org` isn't the only binary cache too! In fact, it's very easy to make your own system serve it's nix store using the `nix-serve` command.
...
```d2
"/nix/var/nix/gcroots": {
  "booted-system/*" { style.multiple: true }
  "current-system/*" { style.multiple: true }
...
So, when comma downloads a command for us, the store entry that package is in (and any dependencies it references) are *not* a descendent of a `gcroot`, meaning when we garbage collect after running comma, the store entry and all of its dependencies (that aren't being used by something else that *is* under `gcroots`), will be deleted.
...
"/nix/store": {
  ...
  "[hash]-system-path" -> "[hash]-etc": Safe!
...
"/nix/var/nix/gcroots"."current-system/sw" -> "/nix/store"."[hash]-system-path": Safe!
"/nix/var/nix/gcroots"."booted-system/sw" -> "/nix/store"."[another-hash]-system-path": Safe!
