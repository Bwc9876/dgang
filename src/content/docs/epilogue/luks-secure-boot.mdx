...
Our first argument to `luksOpen` is the device we want to open with LUKS. The second is the *mapped* name that we'll use to interface with the device.
...
In terms of removing unwanted files each reboot, this is achieved by mounting a `tmpfs` (file system that uses free memory) to `/` instead of your normal drive. Your normal drive is then mounted to `/nix` so the nix store is persisted, from there NixOS handles setting every other path each boot and making a functioning system.
...
Now to set up `/mnt`, unlike the normal installation we won't be mounting our main partition first. Instead, we'll be mounting a tmpfs to mimick our impermanence setup.
...
<Aside>
We're dedicating 4GB of memory to the tmpfs because the initial build will take quite a bit. This is *not* the size we'll use after install.
</Aside>
...
<Aside type="caution">
  Note your keys **must** be in `/mnt/nix/persist/secure/secureboot`, otherwise lanzaboote won't be able to find them.
</Aside>
...
Boot into your system, you'll need to enter the passphrase for your LUKS volumes in order to boot. Don't worry, we'll make this automatic in this section.
...
With that you've enrolled the keys! When you boot next, your initrd will automatically know to try the TPM for decryption keys first, meaning you won't need to enter your password. 
