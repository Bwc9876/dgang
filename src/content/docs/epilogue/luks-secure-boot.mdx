...
UEFI is a (relatively) new standard for firmware to follow. It offers a number of improvements over BIOS one of which being secure boot, but in order to understand how it works we'll need to know how UEFI boots.
...
"TPM SecureBoot DB": { shape: parallelogram }
"Is The Key Valid and not in DBX?" : { shape: diamond }
...
"Firmware" -> "Is The Key Valid and not in DBX?" : When Booting
"Is The Key Valid and not in DBX?" -> "Execute the binary" : Yes
"Is The Key Valid and not in DBX?" -> "SecureBoot violation! Refusing to execute the binary" : No
```
...
Our first argument to `luksOpen` is the device we want to open with LUKS. The second is the *mapped* name that the device we'll use to interface with the device.
...
Usually users will choose to encrypt their main partition and any swap partitions they have set up. The boor partition **should not** be encrypted as the firmware needs to find the bootloader as outlined in the secure boot section.
...
"/dev/sda" <- "luksOpen" : Unlocks the partition
"luksOpen" -> "/dev/mapper/cryptroot" : Creates Map Device
...
Opening the LUKS device with a password every boot can get quite annoying, espacially if you have multiple with different passwords. The trusted platform module (TPM) of our computer instead allows us to store decryption keys for the disk and implicitly *release* them to our initrd on boot, skipping the need to enter the password. This does mean our decryption is now tied to our exact hardware, but becuase LUKS allows for multiple decryption methods we'll still be able to use a password to decrypt the disk as well.

In addition, storing the decryption keys on the TPM allows us to specify conditions that must be met for the TPM releases the keys for decryption. The most interesting of these being PCR `7`, which requires secure boot to be enabled and verified. This means that both our firmware and out actual OS depends on our EFI binary being signed, awesome!
...
In terms of removing unwanted files each reboot, this is achieved by mounting a `tmpfs` (file system that uses RAM) to `/` instead of your normal drive. Your normal drive is then mounted to `/nix` so the nix store is persisted, from there NixOS handles setting every other path each boot and making a functioning system.

## Setting Up Secureboot With Lanzaboote

Alright, now that we understand these features lets implement them as a NixOS module. We'll set up secure boot first as it doesn't need a re-install to set up.
...
This will create our keys in `/var/lib/sbctl` as lanzaboote expect. We can now rebuild our system with the new module and lanzaboote should sign the binaries. We can verify the needed EFI binaries are signed with sbctl as well.

```sh
sudo sbctl verify
```

The files `/boot/EFI/BOOT/BOOTX64.EFI` and `/boot/EFI/Linux/nixos-generation-###-[some-hash].EFI` should be listed as signed, they are then you're good to go!

We've signed our EFI binaries, now we need to inform the platform of our keys and tell it to enforce them. To do this reboot your system into the UEFI interface (you can select "reboot into firmware interface" in the generation selection screen to do this). From here it'll depend on your device, usually under "Security" or "Boot" options there should be an option to enable secure boot, set this to "Enabled" and then looks for options to manage the `PK`, `KEK`, and `DB`. You'll want to clear each of these sections, ensuring they're reset.

<Aside type="caution">
  Do **not** delete DBX! This is a deny list of known exploited keys and clearing it means your system will trust them!
</Aside>
...
<Aside>
  ...
  You *could* not add this flag if you're absolutely certain that your option ROMs don't rely on them, but better safe than sorry.
</Aside>
...
The most drastic of these features is impermanence. This will require a bit more work depending on your specific use cases as you'll need to *manually specify which paths you want to keep*. This includes and application or game save directories.
...
Next, we'll disable `mutableUsers`. Currently we allow the changing of user passwords and other user data. User information like this is stored in `/etc/passwd` and `/etc/shadow`. Instead of persisting these, we'll instruct NixOS to use a hash we generate manually.
...
<Aside>
This guide is having you use the same password for both your user and the root user, feel free to change the paths to be different if you need to. Just remember later you'll need to generate both files!
</Aside>
...
Now, with both your installer flash drive and a drive with (at least) your secure boot keys on it, restart your computer, disable secure boot enforcement (do *not* clear keys as we'll be re-using ones we've already enrolled), and boot into the installer.
...
We'll now format our new partitions for LUKS, similar to chapter 7 `PARTITION_PATH` here will refer to the `/dev` file for your disk.
...
You'll be prompted to enter a passphrase, make sure it's a memorable one as **you will need it if for whatever reason the volume can't be unlocked with TPM**.

In addition, set up LUKS for your swap partition if your set it up.
...
We now need to set up `/mnt`, unlike normal installation we won't be mounting our main partition first, instead we'll mount a tmpfs to mimick our impermanence setup.
...
<Aside>
We're dedicating 4GB of memory to the tmpfs because the build will take quite a bit. This is *not* the size we'll always have.
</Aside>

Now we'll create mount points for our boot and main partitions.
...
Finally, we'll create needed directories
...
Next, plug in and mount your flash drive with your secure boot keys, this will most likely be a `/dev/sdX` path.
...
<Aside type="caution">
  Note your keys **must** be in `/mnt/nix/persist/secure/secureboot`, otherwise lanzaboote won't be able to find it
</Aside>
...
We're now ready to configure the new system, copy your flake (`/etc/flake`) to `/mnt/nix/persist/flake` and set it to be writeable.
...
Finally, edit your `hardware-configuration.nix`. You'll want to remove your old `fileSystems` and `swapDevices` sections and replace them to reflect the impermanence setup you've created.
...
{config, pkgs, lib, modulesPath, ...}: {
  ...
  fileSystems."/" = {
    fsType = "tmpfs";
    # Adjust the size you want for the tmpfs as needed, you shouldn't need to much but it's good to leave overhead
...
With those changes made, we can now build and install our system. We'll be editing our install command a bit here to not prompt for a root password, as we're setting that via the `hashedPasswordFile` option.

```sh
sudo nixos-install --flake .# --root /mnt --no-root-passwd --no-channel-copy
```
...
**Before** booting into the main system, make sure to go into your UEFI settings and enable secure boot. This will both confirm that lanzaboote is still working and also allows us to perform TPM-backed LUKS enrollment.

### Enrolling TPM Keys for LUKS

Boot into your system, you'll need to enter the passphrase for your LUKS volumes you set up in order to boot. Don't worry, we'll make this automatic in this section.
...
With that you've enrolled the keys! When you boot next, your initrd will automatically know to try the TPM for decryption keys first, meaning you won't need to enter your password. In addition, because we used PCR 7 (secure boot state), your TPM will only release the decryption keys if your device is booting using a signed UEFI executable.
...
You've also probably missed a few paths that you want to persist. Keep in mind the impermanence module *will not* be able to override existing paths, so I suggest this general workflow for adding a path to persist.
...
1. Move the directory to a backup location, so we can copy it back later
   ...
   ```
2. Add the path to `environment.persistence`
